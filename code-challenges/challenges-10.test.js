...
const createServer = () => {
  ...
  const app = express();
  app.get('/', (req, res) => {res.status(404).send('oof. Sorry bud, it\'s gone.')});
  app.get('/hello', (req, res) => {res.send('Good after-morning!')});
  app.get('/aboutme', (req, res) => {res.send('a short bio about you')});
  app.get('/favoritefoods', (req, res) => {res.send(['fruits', 'vegetables', 'meats', 'breads'])});
...
const count = (target, input) => {
  let count = 0;
  input.map(arr => {
    let accumulate = arr.reduce((accumulator, value, index) => {
      if (value === target) {accumulator++};
      return accumulator;
    }, 0);
    count += accumulate;
  })
...
const totalSum = (input) => {
  let sum = 0;
  input.forEach(arr => arr.forEach(int => sum += int));
  return sum;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  input.map(arr1 => {
    let arr2 = [];
    arr1.map(int => {
      if (Number.isInteger(int) && int % 5 === 0) {
        arr2.push(Math.pow(2, int))
      };
    })
    arr.push(arr2);
  });
