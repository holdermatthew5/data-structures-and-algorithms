...
const templateWithJQuery = () => {
  starWarsPeople.forEach(obj => {
    ...
    let $clone = $(`<section>${$template}</section`)
    $clone.find('h2').text(obj.name);
    $clone.find('h3').text(obj.height);
    $clone.find('p').text(obj.eye_color);
    $('main').append($clone);
  })
}
...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(str => {
    let cut = str.slice(3);
    let index = cut.indexOf(' ');
    result.push(cut.slice(index + 1));
  })
  return result;
};
