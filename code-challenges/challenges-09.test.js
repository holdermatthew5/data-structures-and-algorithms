...
const mapCurrentEvents = () => {
  let arr1 = currentEvents.news.map(obj2 => new Event(obj2));
  return arr1;
}
...
const countNumberOfElements = (arr) => {
  let arr1 = arr.reduce((accumulator, value, index) => {
    accumulator++;
    return accumulator;
  }, 0)
  return arr1;
...
const returnNames = (arr) => {
  let nameArr = arr.reduce((name, value, index) => {
...
const reversedString = (str) => {
  let arr = str.split('');
  let reversed = arr.reduce((newStr, value, index) => {
    return value + newStr;
  }, '');
  return reversed;
};
